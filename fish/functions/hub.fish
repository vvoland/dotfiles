function hub --argument action --argument ref
    ...
    switch $action
        case blob
            test -z "$repo" && echo "repo is empty" && return 1
            test -z "$digest" && echo "digest is empty" && return 1
            set -l u "https://registry-1.docker.io/v2/$repo/blobs/$digest"
            curl -s -f -H "Authorization: Bearer $token" -L "$u"
        case manifest
            test -z "$repo" && echo "repo is empty" && return 1
            test -z "$tag_or_digest" && echo "tag and digest is empty" && return 1
            set -l u "https://registry-1.docker.io/v2/$repo/manifests/$tag_or_digest"
            curl -v -s -f -H 'Accept: *' -H "Authorization: Bearer $token" -L "$u"
    end
end
